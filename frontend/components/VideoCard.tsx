...
import { Card, CardBody, CardFooter, Image, Chip, Button, Tooltip } from "@heroui/react";
...
export default function VideoCard({ video, onCategorize, isCategorizing, viewMode = "grid" }: VideoCardProps) {
      ...
      <Card className="w-full hover:shadow-lg transition-shadow">
        <CardBody className="p-4">
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1 min-w-0 cursor-pointer" onClick={openYouTube}>
      ...
      <CardBody className="p-0">
        ...
        <div className="p-3 space-y-2 cursor-pointer" onClick={openYouTube}>
          ...
          <div className="flex items-center gap-2 text-xs text-gray-400">
            {video.view_count && <span>{formatViews(video.view_count)} views</span>}
            {video.published_at && (
              <>
                <span>•</span>
                <span>{formatDistanceToNow(new Date(video.published_at), { addSuffix: true })}</span>
